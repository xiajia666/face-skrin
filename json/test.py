...
def p_annotation(data, data_coco, cnt):
    ...
    for i in range(len(data['shapes'])):
        ...
        annotation['segmentation'] = [
            list(np.asarray(data['shapes'][i]['points']).flatten())]  # data['shapes'][0]['points']
        ...
        cat_list_dict = [cat for cat in data_coco['categories'] if
                         cat['name'] == data['shapes'][i]['label'].split('_')[1]]  # 注意这里是跟标注时填类别的方式有关
        print(cat_list_dict)
...
if __name__ == '__main__':
    ...
    for json_path in json_list:
        print(os.path.join(root_dir, json_path))
        if json_path.split('.')[-1] == 'json':
            data_coco, file_name = Labelme2coco(os.path.join(root_dir, json_path), cnt)
            # 保存json文件
            print(data_coco)
            print(file_name)
            json.dump(data_coco, open('./train/%s.json' % file_name.split('.')[0], 'w'), indent=4)